...
$body = @{ workspace = $Workspace; recreate = [bool]$Recreate } | ConvertTo-Json

try {
  $res = Invoke-RestMethod -Method Post -Uri "$BaseUrl/workspace/index" -ContentType "application/json" -Body $body
  ...
  if ($errorBody -match "workspace_not_registered") {
    ...
    $retry = Invoke-RestMethod -Method Post -Uri "$BaseUrl/workspace/index" -ContentType "application/json" -Body $body
